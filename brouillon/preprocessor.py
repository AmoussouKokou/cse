...
class Preprocessor(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y=None):
        ...
        if isinstance(self.imputer, DropMvImputer) and hasattr(self.imputer, "how"):
            if self.imputer.how is not None:
                self.row_with_mv = X.isnull().any(axis=1) if self.imputer.how == "any" else X.isnull().all(axis=1)
            else:
                self.row_with_mv = pd.Series(False, index=X.index)
        else:
            self.row_with_mv = pd.Series(False, index=X.index)
            # print(self.rows_to_remove)
...
if __name__ == "__main__":
    ...
    data.to_csv("fichier_exporte.csv", index=False, encoding="utf-8")